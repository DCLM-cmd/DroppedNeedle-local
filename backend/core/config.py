...
# only falls back when the key is ABSENT, and a Dockerfile writing
# ``ENV COMMIT_TAG=${COMMIT_TAG}`` from an ARG nothing passes leaves the variable
# present and EMPTY, so the default never applies. The User-Agent then reads
# "DroppedNeedleApp/ (...)" with no version at all, which MetaBrainz (ListenBrainz,
# MusicBrainz, Cover Art Archive) throttles: they require an application name AND a
# version. Anything that reads a build stamp must go through here.
