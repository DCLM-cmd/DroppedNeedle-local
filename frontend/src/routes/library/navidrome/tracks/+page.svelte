<script lang="ts">
	...
	let data = $state<NavidromeTrackPage>({ items: [], total: 0, offset: 0, limit: PAGE_SIZE });
	...
	function isTrackPlaying(track: NavidromeTrackInfo): boolean {
		return (
			playerStore.isPlaying &&
			playerStore.currentQueueItem?.trackSourceId === track.navidrome_id &&
			playerStore.currentQueueItem?.sourceType === 'navidrome'
		);
	}
...
<div class="container mx-auto max-w-7xl px-4 py-6">
	...
	>
		...
		<h1 class="text-2xl font-bold">Navidrome Tracks</h1>
		{#if data.total > 0}
