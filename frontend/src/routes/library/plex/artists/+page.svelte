<script lang="ts">
	...
	import type { PlexArtistPage } from '$lib/types';

	const PAGE_SIZE = 48;

	const ARTIST_SORT_OPTIONS = [
		{ value: 'titleSort', label: 'Name' },
		{ value: 'addedAt', label: 'Recently Added' },
		{ value: 'lastViewedAt', label: 'Recently Played' }
	];

	let loading = $state(true);
	let artists = $state<PlexArtistPage>({ items: [], total: 0, offset: 0, limit: PAGE_SIZE });
...
<div class="container mx-auto max-w-7xl px-4 py-6">
	...
	>
		...
		<h1 class="text-2xl font-bold">Plex Artists</h1>
		{#if artists.total > 0}
	...
	{:else}
		<div class="rounded-xl bg-base-100/40 p-5 shadow-sm" use:reveal>
			<div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
				{#each artists.items as artist (artist.plex_id)}
					<SourceArtistCard
						name={artist.name}
